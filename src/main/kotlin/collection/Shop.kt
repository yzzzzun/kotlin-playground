...
fun Shop.getWaitingCustomers(): List<Customer> = this.customers
    .filter { customer ->
        customer.orders.any { order -> !order.isDelivered }
    }

fun Shop.countProductSales(product: Product): Int = this.customers
    .sumOf { it.orders.sumOf { order ->
        order.products.count{p -> p == product}
    }
}


fun Shop.getCustomers(minAmount: Double): List<Customer> = this.customers
    .filter { customer ->
//        customer.orders
//            .flatMap { order -> order.products }
//            .sumOf { product -> product.price } >= minAmount
        customer.orders.sumOf {
            order -> order.products.sumOf { it.price }
        } >= minAmount
    }
...
class ShopFunctionsTests {
    ...
    fun `getWaitingCustomers should not return customers all orders delivered`() {
        ...
        // then
        assertEquals(listOf<Customer>(), result)
    }
    ...
    fun `getCustomers should get customers with orders with total price greater than minAmount`() {
        ...
        assertEquals(listOf<Customer>(), shop.getCustomers(30.0))
    }
}